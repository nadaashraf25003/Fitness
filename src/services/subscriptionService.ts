...
export const subscriptionService = {
  ...
  submitRequest(
    ...
    const newRequest: SubscriptionRequest = {
      ...request,
      id: `req-${Date.now()}`,
      status: request.simulatePayment ? 'approved' : 'pending',
      createdAt: new Date().toISOString(),
    };
    ...
    apiClient
      .post('/fitness/request', {
        ...
        simulate_payment: request.simulatePayment === true,
        member: {
      ...
      .then((res) => {
        if (res.data && res.data.request_id) {
          ...
          if (idx !== -1) {
            ...
            current[idx].status = request.simulatePayment ? 'approved' : 'pending';
