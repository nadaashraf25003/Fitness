...
export const SubscriptionRequestModal: React.FC<SubscriptionRequestModalProps> = ({
  ...
  // Validate the simulated card form. This demo never sends card data to the server.
  const validateStep2 = () => {
    const errs: Record<string, string> = {};
    if (formData.paymentMethod === 'Visa') {
      const cleanNum = cardData.cardNumber.replace(/\s/g, '');
      if (!cleanNum.startsWith('4') || cleanNum.length !== 16) errs.cardNumber = 'Enter a valid 16-digit Visa card number';
      if (!cardData.cardHolder.trim()) errs.cardHolder = 'Cardholder name is required';
      if (!cardData.expiry || cardData.expiry.length < 5) errs.expiry = 'Valid MM/YY required';
      if (!cardData.cvv || cardData.cvv.length < 3) errs.cvv = '3 or 4-digit CVV required';
    }
    setErrors(errs);
    return Object.keys(errs).length === 0;
  };
  ...
  // Run the local card-payment simulation.
  ...
  // Small confirmation sequence while the request is saved to the system.
  const triggerGatewayProcessing = async () => {
    ...
    setProcessingStatus('Preparing secure demo payment...');
    await new Promise((r) => setTimeout(r, 600));

    // Stage 2: Clearing Network
    setProcessingProgress(50);
    if (formData.paymentMethod === 'Visa') {
      setProcessingStatus('Authorizing simulated Visa payment...');
    } else {
      setProcessingStatus('Recording cash payment in the gym system...');
    }
    await new Promise((r) => setTimeout(r, 700));

    // Stage 3: Bank Settlement
    setProcessingProgress(80);
    setProcessingStatus('Recording payment and activating membership...');
    await new Promise((r) => setTimeout(r, 600));

    // Stage 4: Success Completion
    setProcessingProgress(100);
    setProcessingStatus('Payment recorded successfully. Creating your invoice...');
    ...
    <Modal
      ...
      title={
        currentStep === 'success'
          ...
          : currentStep === 'otp'
          ? 'Bank Identity Check (Demo)'
          : currentStep === 'payment'
          ? 'Secure Demo Checkout'
          : currentStep === 'processing'
          ? 'Authorizing Demo Transaction...'
          : `Subscribe to GEM — ${selectedPlan?.name || 'Plan'}`
      }
      subtitle={
        currentStep === 'success'
          ? 'Your demo transaction was recorded successfully and your membership is active.'
          : currentStep === 'otp'
          ? 'Enter the demo verification code to complete the simulated payment.'
          : currentStep === 'payment'
          ? 'Complete the demo card details to activate your gym enrollment.'
          : currentStep === 'processing'
          ? 'Recording the demo transaction in the gym system...'
    ...
    >
        ...
        <div className="space-y-5 py-1">
          ...
          <div className="text-center space-y-1.5">
            ...
            <p className="text-xs text-text-muted max-w-md mx-auto">
              Your membership for <strong>{submittedRequest.planName}</strong> is authorized and active from{' '}
          ...
          <div className="p-5 rounded-2xl bg-surface-card border border-border-subtle relative overflow-hidden shadow-xl space-y-4">
            ...
            <div className="flex items-start justify-between pb-3 border-b border-border-subtle">
              <div>
                ...
                <p className="text-[10px] text-text-muted">Demo payment receipt • No real bank transaction</p>
              </div>
              <div className="text-right">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
              ...
              <div className="p-2.5 rounded-xl bg-surface-elevated/60 border border-border-subtle">
                <span className="text-text-muted block text-[10px] uppercase font-semibold">Authorization</span>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <span className="text-[10px] text-text-muted block">
                    Show this pass to the front desk to complete your first check-in.
                  </span>
                </div>
              </div>
          ...
          <div className="flex flex-wrap gap-2.5 pt-1">
            ...
            >
              {emailSentToast ? 'Receipt Sent! ✓' : 'Email Receipt'}
            </Button>
      ...
      {currentStep === 'payment' && (
        <form onSubmit={handleInitiatePayment} className="space-y-4">
          ...
          <div className="p-3 rounded-xl bg-brand-primary/10 border border-brand-primary/30 text-xs text-text-muted flex items-start gap-2">
            <Info className="w-4 h-4 mt-0.5 flex-shrink-0 text-brand-primary" />
            <span>
              Demo checkout: this flow looks like a real payment and records the payment as paid inside the gym system,
              but it does not contact a bank or charge real money. Use demo card details only.
            </span>
          </div>

          {/* Simulated card fields — no card data is sent to or stored by the backend. */}
          ...
          <div className="pt-2">
            ...
            >
              {formData.paymentMethod === 'Cash'
                ? `Confirm Demo Cash Payment ($${selectedPlan?.price})`
                : `Authorize Demo Visa Payment ($${selectedPlan?.price})`}
            </Button>
          </div>
        </form>
      )}
